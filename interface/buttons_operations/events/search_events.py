...
class Search_Events(Frame):
    def __init__(self, parent, controller):
        ...
        # Content
        content_label = Label(
            self, text="SEARCH EVENT", bg="#333333", fg="#BDFDC0", font=("Arial", 20, "bold"), justify="center"
        )
        ...
        add_button = Button(
            ...
            width=15, height=2,
            relief="flat",
            command=lambda: self.cautare_eveniment(self.name_entry)
        )
        add_button.pack()
    ...
    def cautare_eveniment(self, name_entry):
        conn = get_connection()
        cursor = conn.cursor()
        try:
        ...
        for eveniment in all_evenimente:
            if nume == eveniment[0]:
                ...
                self.confirmation_label.config(text=f"The event named {nume} exists", fg="#00FF00")
                break

        if not eveniment_gasit:
            self.confirmation_label.config(text=f"The event named {nume}\n doesn't exist")
        conn.close()
        cursor.close()
